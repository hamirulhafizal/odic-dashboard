<table>
    <thead>
      <tr>
        <th colspan="4">Order #{{ $users->id}}</th>
        <th colspan="4">Remark: {{ $users->remark}}</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td colspan="8">Contact Number: +65{{ $users->phone}}</td>
      </tr>
      <tr>
        <th>Product ID</th>
        <th>Product Name</th>
    ...
    @endforeach
      <tr>
        <td colspan="4">Delivery Address - {{ $users->address_1 }}, {{ $users->level }}-{{ $users->unit}}, {{ $users->country}}</td>
        <td colspan="4">Total Delivery Charge ${{ $users->total_delivery_fee }}</td>
      </tr>
      <tr>
        <td colspan="4">Postal Code - {{ $users->postcode }}</td>
        <td colspan="4">Total ${{ $users->total }}</td>
      </tr>
    </tbody>
    </table>
